...
def reopen_trip_leg(db: Session, leg_id: int, user_id: int):  # <--- AUDITORÍA PARAM
    ...
    if leg.status != models.TripStatus.LIQUIDADO:
        raise ValueError(
            f"El tramo tiene estatus '{leg.status}', debe estar 'liquidado' para reabrirse."
        )
    ...
    cxc_list = (
        ...
        .filter(
            ...
            == False,  # Es la factura interna de operaciones
        )
        .all()
    )
    ...
    for cxc in cxc_list:
        ...
        if cxc.estatus in [
            ...
            raise ValueError(
                f"No se puede reabrir la liquidación. La Cuenta por Cobrar ({cxc.folio_interno}) "
    ...
    leg.status = (
        models.TripStatus.CERRADO
    )  # Regresamos el estatus a Cerrado (previo a liquidación)

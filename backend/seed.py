...
def seed_roles(db):
    ...
    for r in roles:
        get_or_create(
            ...
            models.Role,
            name_key=r["key"],
            defaults={"nombre": r["nombre"], "permisos": r["permisos"]},
        )
    print("✓ Roles creados")
...
def seed_client_sub_tariffs(db):
    ...
    # Subcliente
    sub1, _ = get_or_create(
        db,
        models.SubClient,
        nombre="Planta Norte Monterrey",
        client_id=client.id,
        defaults={
            "direccion": "Blvd. Diaz Ordaz 500",
            "ciudad": "Monterrey",
            "estado": "NL",
        },
    )
    ...
    get_or_create(
        ...
        defaults={"tarifa_base": 28500.00, "vigencia": date(2026, 12, 31)},
    )
...
def seed_units(db):
    ...
    u2, _ = get_or_create(
        ...
        defaults={
            ...
            "status": models.UnitStatus.EN_RUTA,
        },
    )
...
def seed_tires(db, unit):
    # Llanta 1 (Montada)
    t1, created = get_or_create(
        db,
        models.Tire,
        codigo_interno="LL-1001",
        defaults={
            "marca": "Michelin",
            "modelo": "X Multi",
            "medida": "295/80R22.5",
            "profundidad_original": 18.0,
            "profundidad_actual": 15.5,
            "unit_id": unit.id,
            "posicion": "Eje 1 Izq",
    ...
    if created:
        ...
        h2 = models.TireHistory(
            ...
            unit_id=unit.id,
            unidad_economico=unit.numero_economico,
            posicion="Eje 1 Izq",
            responsable="Taller",
        )
        db.add_all([h1, h2])
    ...
    t2, _ = get_or_create(
        db,
        models.Tire,
        codigo_interno="LL-1002",
        defaults={
            "marca": "Bridgestone",
            "modelo": "R268",
            "medida": "11R22.5",
            "profundidad_original": 16.0,
            "profundidad_actual": 16.0,
            "unit_id": None,
            "posicion": None,  # En almacén
            "fecha_compra": date(2024, 5, 10),
...
def seed_maintenance(db):
    # 1. Mecánicos
    mec1, _ = get_or_create(
        db,
        models.Mechanic,
        nombre="Carlos Ruiz",
        defaults={"especialidad": "Motor Diesel", "activo": True},
    )
    mec2, _ = get_or_create(
        db,
        models.Mechanic,
        nombre="Pedro Sánchez",
        defaults={"especialidad": "Sistema Eléctrico", "activo": True},
    )
    ...
    if prov1:
        get_or_create(
            ...
            defaults={
                ...
                "fecha_vencimiento": date(2024, 1, 25),
                "concepto": "Consumo Diesel Semana 2",
                "estatus": "pendiente",
            },
        )

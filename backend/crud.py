...
def get_clients(db: Session, skip: int = 0, limit: int = 100) -> List[models.Client]:
    ...
    return db.query(models.Client)\
        .options(
            joinedload(models.Client.sub_clients)
            .joinedload(models.SubClient.tariffs)
        )\
        .offset(skip)\
        .limit(limit)\
        .all()
...
def get_operators(
    ...
    for op in operators:
        ...
        medical_days = (op.medical_check_expiry - today).days if op.medical_check_expiry else -999
...
def get_trips(
    ...
    trips = query.order_by(models.Trip.start_date.desc()).offset(skip).limit(limit).all()
...
def create_trip(db: Session, trip: schemas.TripCreate) -> models.Trip:
    ...
    if unit.status != models.UnitStatus.DISPONIBLE:
        raise ValueError(f"Unidad {unit.numero_economico} no está disponible (status: {unit.status})")
    if unit.documentos_vencidos > 0:
        raise ValueError(f"Unidad {unit.numero_economico} tiene {unit.documentos_vencidos} documentos vencidos")
    ...
    if operator.status != models.OperatorStatus.ACTIVO:
        raise ValueError(f"Operador {operator.name} no está activo (status: {operator.status})")
...
def update_trip_status(
    ...
    if db_trip:
        ...
        event = models.TripTimelineEvent(
            ...
            event=f"Status cambiado de {old_status} a {status}" + (f" - {location}" if location else ""),
            event_type="checkpoint" if status == models.TripStatus.ENTREGADO else "info"
        )
        db.add(event)

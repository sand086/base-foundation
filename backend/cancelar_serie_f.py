...
from app.models.models import ReceivableInvoice, ReceivableInvoicePayment, InvoiceStatus
...
def cancelar_rep_incorrecto():
    ...
    try:
        # 1. Buscar en BD en ambas tablas (Facturas y Pagos)
        factura_p = (
            db.query(ReceivableInvoice)
            .filter(ReceivableInvoice.uuid == uuid_rep)
            .first()
        )
        pago = (
        ...
        if factura_p:
            print(
                f"✅ REP encontrado en tabla Facturas. Folio: {factura_p.folio_interno}"
            )
        if pago:
            print(f"✅ REP encontrado en tabla Pagos. Monto: ${pago.monto}")
        if not factura_p and not pago:
            print(
...
<soap-env:Envelope xmlns:soap-env="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soap-env:Body>
    <ns0:cancelar xmlns:ns0="http://timbrado.ws.cfdi.solucionfactible.com">
      ...
      <ns0:uuids>{uuid_rep}|02|</ns0:uuids>
      ...
      <ns0:contrasenaCSD>{contrasena_csd}</ns0:contrasenaCSD>
        ...
        print(
            f"⏳ Ejecutando petición SOAP a Solución Factible para cancelar el REP..."
        )
        ...
        if (
            ...
            print(f"✅ ¡ÉXITO! El REP {uuid_rep} fue enviado a cancelación en el SAT.")

            # Sincronizar en BD si existía
            if factura_p:
            ...
            print("⚠️ Revisa el mensaje de error devuelto por el PAC:")
            print(response.text)

    except Exception as e:
        db.rollback()
        print(f"❌ Error crítico procesando la cancelación: {e}")

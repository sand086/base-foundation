...
export const useSecurityNotifications = () => {
  const sendSecurityNotification = ({ event, details }: SecurityNotificationParams) => {
    ...
    switch (event) {
      case 'password_reset':
        ...
        console.log(`[SECURITY] ${timestamp} - Password reset email sent to: ${details.userEmail}`);
        break;
      ...
      case 'two_factor_disabled':
        ...
        console.log(`[SECURITY] ${timestamp} - 2FA disabled for user: ${details.userName}. Email notification sent.`);
        break;
      ...
      case 'two_factor_reset':
        ...
        console.log(`[SECURITY] ${timestamp} - 2FA reset for user: ${details.userName}. Email notification sent.`);
        break;
      ...
      case 'trip_stopped':
        ...
        console.log(`[OPERATIONS] ${timestamp} - Trip ${details.tripId} STOPPED. Client notification sent to: ${details.clientName}`);
        break;

      case 'trip_incident':
        toast.error('⚠️ Incidencia Reportada - Notificación al Cliente', {
          description: `Incidencia en viaje ${details.tripId}. Motivo: ${details.reason || 'No especificado'}. Cliente notificado.`,
          duration: 8000,
        });
        console.log(`[OPERATIONS] ${timestamp} - Trip ${details.tripId} INCIDENT: ${details.reason}. Client notification sent.`);
        break;
      ...
      case 'forced_assignment':
        ...
        console.log(`[AUDIT] ${timestamp} - FORCED ASSIGNMENT by ${details.adminName}. Reason: ${details.reason}`);
        break;

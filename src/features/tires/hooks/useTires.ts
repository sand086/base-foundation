...
export const useTires = () => {
  ...
  const fetchTires = useCallback(async () => {
    ...
    try {
      ...
      console.error(error);
      console.error("Error al cargar inventario de llantas");
    } finally {
      setIsLoading(false);
    }
  }, []);
    ...
    try {
      ...
      const msg =
        error instanceof ApiError
          ? error.body?.detail
          : "Error al crear llanta";
      console.error(msg || "Error al crear llanta");
      return false;
    }
  };
  ...
  const updateTire = async (id: number, data: any) => {
    try {
      ...
      const msg =
        error instanceof ApiError
          ? error.body?.detail
          : "Error al actualizar llanta";
      console.error(msg || "Error al actualizar llanta");
      return false;
    }
  };
  ...
  const deleteTire = async (id: number) => {
    try {
      ...
      const msg =
        error instanceof ApiError
          ? error.body?.detail
          : "No se puede eliminar la llanta";
      console.error(msg || "No se puede eliminar la llanta");
      return false;
    }
  };

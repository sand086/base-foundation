...
import { validateImportData, ValidationResult, getErrorPreview } from './validationUtils';
  ...
  const handleProcessUpload = () => {
    if (validationResult && !validationResult.isValid) {
      toast.error('Archivo con errores', {
        description: 'Corrige los errores antes de procesar la carga.',
      });
      return;
    }
    ...
    setTimeout(() => {
      ...
      toast.success('Carga procesada exitosamente', {
        description: `${validationResult?.validRowCount || previewData.length - 1} registros importados correctamente.`,
      });
    }, 2000);
  };
            ...
            {step === 'download' && (
              <div className="space-y-4">
                ...
                <div className="border rounded-lg p-4 space-y-3">
                  <h4 className="text-sm font-medium">Columnas de la plantilla:</h4>
                  <div className="flex flex-wrap gap-2">
                    {importType.columns.map((col, i) => (
                      ...
                      >
                        {col.name}
                        {col.required && <span className="ml-1 text-[10px]">*</span>}
                      </Badge>
                    ))}
                  </div>
                ...
                <div className="border rounded-lg p-4 space-y-2">
                  <h4 className="text-sm font-medium">Referencia de formatos:</h4>
                  <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                    {importType.columns.filter(c => c.type === 'enum').map((col, i) => (
                    ...
                    {importType.columns.filter(c => c.type === 'date').length > 0 && (
                ...
                {validationResult && (
                  <div className={cn(
                    ...
                    <div className="flex-1">
                      <p className={cn(
                        "text-sm font-medium",
                        validationResult.isValid ? "text-status-success" : "text-status-danger"
                      )}>
                        {validationResult.isValid 
                          ? 'Archivo válido' 
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {validationResult.validRowCount} de {validationResult.totalRowCount} registros válidos.
                      </p>
                      
                      {/* Show first errors */}
                      {!validationResult.isValid && (
                        <div className="mt-3 space-y-1">
                          {getErrorPreview(validationResult.errors).map((error, i) => (
                            <div key={i} className="text-xs flex items-start gap-2 text-status-danger">
            ...
            {step === 'processing' && (
              <div className="py-12 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Enviando {validationResult?.validRowCount || previewData.length - 1} registros al servidor.
                  </p>
                </div>
              </div>
            )}
            ...
            {step === 'complete' && (
              <div className="py-8 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {validationResult?.validRowCount || previewData.length - 1} registros fueron importados exitosamente.
                  </p>
                </div>

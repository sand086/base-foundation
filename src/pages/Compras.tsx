...
import { mockPurchaseOrders } from "@/features/payables/data"; // Faltaba importar la data
import { PurchaseOrderWizard } from "@/features/payables/components/PurchaseOrderWizard"; // Corregida la ruta y el nombre
import {
  ReceiveOrderModal,
  ConvertToCxPModal,
} from "@/features/payables/components/ReceiveOrderModal"; // Corregida la ruta
import { printOrderPDF } from "@/features/payables/printOrderPDF";

export default function Compras() {
  const navigate = useNavigate();
  // Ahora mockPurchaseOrders sí está importado
  ...
  const kpis = useMemo(
    () => ({
      ...
      pendientes: orders.filter((o) => o.estatus === "pendiente_aprobacion")
        .length,
      aprobadas: orders.filter((o) => o.estatus === "aprobada").length,
      sinConvertir: orders.filter(
        (o) => o.estatus === "recibida" && !o.convertidoACxP,
      ).length,
    }),
    [orders],
  );
  ...
  const handleConvert = (orderId: string) => {
    setOrders(
      orders.map((o) =>
        o.id === orderId ? { ...o, convertidoACxP: true } : o,
      ),
    );
  ...
  const columns: ColumnDef<PurchaseOrder>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (v) => (
          <Badge className={getOrderTypeColor(v)}>{getOrderTypeLabel(v)}</Badge>
        ),
      },
      ...
      { key: "solicitante", header: "Solicitante" },
      {
        ...
        type: "number",
        render: (v, r) => (
          <span className="font-mono">{formatCurrency(v, r.moneda)}</span>
        ),
      },
      {
        key: "estatus",
        ...
        render: (_, row) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => printOrderPDF(row)}>
                ...
                <DropdownMenuItem
                  onClick={() => {
                    setOrders(
                      orders.map((o) =>
                        o.id === row.id
                          ? { ...o, estatus: "pendiente_aprobacion" }
                          : o,
                      ),
                    );
              ...
              {row.estatus === "pendiente_aprobacion" && (
              ...
              {row.estatus === "recibida" && !row.convertidoACxP && (
                <DropdownMenuItem
                  onClick={() => {
                    setSelectedOrder(row);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 🚀 Usando el nuevo nombre PurchaseOrderWizard */}
      <PurchaseOrderWizard
        open={wizardOpen}
        onOpenChange={(o) => {
          setWizardOpen(o);
